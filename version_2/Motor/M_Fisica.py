...
class M_Fisica(object):
    ...
    def esMovimientoPosible(self, focalizarEn = ""):
        ...
        if cuerpoChoqueAbsolutoSiguiente.x >= si_x and cuerpoChoqueAbsolutoSiguiente.y >= si_y:
            if cuerpoChoqueAbsolutoSiguiente.x + self.cuerpoChoque.ancho <= id_x and cuerpoChoqueAbsolutoSiguiente.y + self.cuerpoChoque.alto <= id_y:
                movimientoPosible = True
        return movimientoPosible
    # ESTOY COLISIONANDO CON CIERTO TIPO DE ENTIDAD
    def buscarColision(self, id = "", tipo = "", siguiente = False):
        encontre = False
        indiceEntidad = 0
        if (not siguiente):
            # BUSCO EN EL GRUPO DE COLISIONES ACTUALES
            while (not encontre) and (indiceEntidad < len(self.colisionesActuales)):
                entidad = self.colisionesActuales[indiceEntidad]
                if (tipo != "" and entidad.idTipo == tipo) or (id != "" and entidad.id == id):
                    encontre = True
                indiceEntidad += 1
        else:
            # BUSCO EN EL GRUPO DE ENTIDADES CERCANAS
            while (not encontre) and (indiceEntidad < len(self.entidadesCercanas)):
                entidad = self.entidadesCercanas[indiceEntidad]
                if (tipo != "" and entidad.idTipo == tipo) or (id != "" and entidad.id == id):
                    if (self.getCuerpoChoqueAbsolutoSiguiente().intersect(entidad.getCuerpoChoqueAbsoluto())):
                        encontre = True
                indiceEntidad += 1
        return encontre
    # DISTANCIA CON DETERMINADA ENTIDAD
    def getDistanciaAEntidad(self, entidad):
        dx = entidad.posicion.x - self.posicion.x
        dy = entidad.posicion.y - self.posicion.y
        distancia = (dx**2 + dy**2) ** .5
        return distancia
    ...
    # COLISIONES ACTUALES
    def getColisionesActuales(self):
        return self.__colisionesActuales
    def setColisionesActuales(self, param):
        self.__colisionesActuales = param
    colisionesActuales = property(getColisionesActuales, setColisionesActuales)
    # ENTIDADES CERCANAS
    def getEntidadesCercanas(self):
        return self.__entidadesCercanas
    def setEntidadesCercanas(self, param):
        self.__entidadesCercanas = param
    entidadesCercanas = property(getEntidadesCercanas, setEntidadesCercanas)