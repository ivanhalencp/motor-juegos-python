...
class M_Escenarios(object):
    ...
    def entidadEnEscena(self, entidad, margen = 0):
        inferiorDerecho = False
        superiorIzquierdo = False
        if entidad.posicion.x < (self.ventana.x + self.ventana.ancho + margen) and entidad.posicion.y < (self.ventana.y + self.ventana.alto + margen):
            inferiorDerecho = True
        if entidad.posicion.x > self.ventana.x - margen and entidad.posicion.y > self.ventana.y - margen:
            superiorIzquierdo = True
        #if (superiorIzquierdo and inferiorDerecho and entidad.id == "bot"):
        #    print "en escena -> id:" + entidad.id
        return (inferiorDerecho and superiorIzquierdo)
