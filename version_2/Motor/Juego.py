...
class Juego(object):
    ...
    def addEntidad(self, entidad, idCapa = -1):
        entidad.profundidad = len(self.entidades)
        # SI SE PASA UN ID DE CAPA
        # - SETEO CAPA A LA ENTIDAD
        # - ACTUALIZO CANTIDAD DE CAPAS SI ES NECESARIO
        # - AGREGO LA ENTIDAD A LA CAPA PASADA
        if idCapa != -1:
            entidad.idCapa = idCapa
            # if len(self.capas) < idCapa + 1:
            while (len(self.capas) < idCapa + 1):
                arrCapas = []
                self.capas.append(arrCapas)
            self.capas[idCapa].append(entidad)
        # AGREGO LA ENTIDAD A LA LISTA DE ENTIDADES
    ...
    def logicaEntidades(self):
        for entidad in (self.entidades):
            if entidad.activo and (self.mEscenarios.entidadEnEscena(entidad, 32) or (entidad.logicaFueraEscena)):
                entidad.logica()
    ...
    def animar(self):
        for entidad in (self.entidades):
            if entidad.activo and (self.mEscenarios.entidadEnEscena(entidad, 32) or entidad.animarFueraEscena):
                entidad.animar()
    # APLICAR FISICA ENTIDADES
    def aplicarFisica(self):
        for entidad in (self.entidades):
            if entidad.activo and (self.mEscenarios.entidadEnEscena(entidad, 32) or entidad.aplicarFisicaFueraEscena):
                entidad.aplicarFisica()
    # GRAFICAR TODO
    def graficar(self):
        # GRAFICAR FONDO
        ajustePosicion = self.mEscenarios.logicaCamara()
        self.mEscenarios.blit(self.pantalla)
        # GRAFICAR ENTIDADES POR CAPAS
        if len(self.capas) > 0:
            for capa in self.capas:
                for entidad in capa:
                    if entidad.activo and (self.mEscenarios.entidadEnEscena(entidad, 32) or entidad.graficarFueraEscena):
                        # SI LA CAMARA SE MOVIO ENTONCES AJUSTO POSICION DE CADA ENTIDAD
                        entidad.blit(self.pantalla, ajustePosicion)
        else:
            # GRAFICAR ENTIDADES (SI NO MANEJA CAPAS)
            for entidad in (self.entidades):
                if entidad.activo and (self.mEscenarios.entidadEnEscena(entidad, 32) or entidad.graficarFueraEscena):
                    # SI LA CAMARA SE MOVIO ENTONCES AJUSTO POSICION DE CADA ENTIDAD
                    entidad.blit(self.pantalla, ajustePosicion)
    ...
    # CAPAS (CONJUNTO DE CONJUNTOS DE ENTIDADES JUEGO)
    def getCapas(self):
        return self.__capas
    def setCapas(self, param):
        self.__capas = param
    capas = property(getCapas, setCapas)
    # PANTALLA
